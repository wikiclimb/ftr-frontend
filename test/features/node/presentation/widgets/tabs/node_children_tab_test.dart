      ...
      MaterialApp(
        ...
        home: Scaffold(
          body: MultiBlocProvider(
            ...
            child: NodeChildrenTab(parentNode: parentNode),
          ),
        ),
      ),
    );
...
void main() {
  ...
  group('without node data', () {
    ...
    testWidgets('renders an node list', (tester) async {
      when(() => authBloc.state).thenAnswer(
        (_) => AuthenticationUnauthenticated(),
      );
      await tester.pumpIt(bloc, authBloc, nodes.first);
      expect(find.byType(NodeList), findsOneWidget);
      expect(
        find.byKey(Key('nodeDetailsScreen_nodeChildrenTab_addChild_fab')),
        findsNothing,
      );
