...
void main() {
  ...
  setUpAll(() async {
    ...
    sl.registerFactoryParam<NodeListBloc, Node?, void>(
      (parentNode, _) => mockNodeListBloc,
    );
  ...
  group('image tab', () {
    testWidgets('renders node images', (tester) async {
      ...
      await tester.pump(Duration(seconds: 1));
      expect(
        find.byKey(Key('nodeDetailsScreen_imageListTab_safeArea')),
        findsOneWidget,
      );
  ...
  group('bottom navigation', () {
    testWidgets('can navigate tabs', (tester) async {
      when(() => mockNodeListBloc.state).thenAnswer(
        (_) => NodeListState(
      ...
      expect(find.byType(NodeChildrenTab), findsOneWidget);
    });
  });
}
